...
export default function ReviewDataPage() {
  const steps = [
    ...
    { name: "Review your data", completed: false, active: true, subItems: [
      { name: "Grains", status: "Needs Review" },
      { name: "Coconut", status: "Needs Review" }
    ]},
  ];

  const dataPoints = [
    { name: "Overall Spend", status: "Available", date: "27 Dec, 2025", action: "Validate", subItems: [
      "Spend by Location",
      "Spend by Supplier",
      "Volume by Supplier",
      "Volume by Geography",
    ...
    { name: "Supply Master", status: "Available", date: "27 Dec, 2025", action: "View" },
    { name: "Contracts", count: 5, status: "0 Items", date: "27 Dec, 2025", action: "Upload" },
    { name: "Category Playbook", status: "0 Items", date: "27 Dec, 2025", action: "Upload" },
    { name: "Other", status: "5 Items", date: "27 Dec, 2025", action: "View" },
  ];
    ...
    <div className="relative flex min-h-screen w-full overflow-hidden bg-[#F8FBFE]">
      ...
      <div className="relative z-10 flex flex-1 flex-col p-8 lg:p-10">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          <Link href="/setup/summary">
            <Button className="h-10 rounded-full bg-[#1A1C1E] px-5 text-sm font-medium text-white transition-all hover:bg-black">
              Next Category: Oils
                      ...
                      <div className="ml-9 space-y-2 pb-2 pr-4">
                        {step.subItems.map((item, i) => (
                          <div key={i} className="flex items-center justify-between py-1">
                             ...
                             <span className="text-[11px] font-bold text-red-500/80">{item.status}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="grid grid-cols-4 gap-4">
               <div className="rounded-[32px] bg-white p-6 shadow-sm ring-1 ring-black/[0.03]">
                  ...
                  <div className="mt-2 flex items-baseline gap-2">
                    <span className="text-2xl font-semibold text-[#1A1C1E]">$ 3.3M</span>
                    <span className="text-[11px] text-gray-400">Jun '24 - May '25</span>
                  </div>
               </div>
                              ...
                              {row.subItems && row.subItems.map((sub, sIdx) => (
                                 <tr key={sIdx} className="bg-gray-50/20 group transition-colors hover:bg-gray-50/50">
                                    <td className="py-4 pl-14">
                                       <span className="text-[14px] text-gray-600">{sub}</span>
                                    </td>
                                    <td className="py-4">
